...
interface IHabilidash {
    ...
    function changeStakeStatus(address _freelancerAddress, StakeStatus _newStatus) external;
}

contract HStaking is Ownable, Initializable, ReentrancyGuard {
    using SafeERC20 for IERC20;  
    IERC20 public stakeToken; // stake token address.
    IERC20 public rewardToken;
    IHabilidash public habilidash;
    ...
    */
    ...
    mapping(uint256 => uint256) public reward; // Mapping to individual transactions for a user.
    ...
    * @param token address.
    */
    function initialize(IERC20 _stakeToken, IERC20 _rewardToken, IHabilidash _habilidash) public initializer {
        stakeToken = _stakeToken;
        rewardToken = _rewardToken;
        habilidash = _habilidash;
        reward[30 days] = 200;
        reward[90 days] = 500;
        reward[180 days] = 1200;
    }
    ...
    */
    function stake(uint256 _time, uint256 _amount) external nonReentrant {
        ...
        require(_amount != 0, "HStake: Null amount!");
        require(_time != 0, "HStake: Null time!");
        require(reward[_time] != 0, "HStake: Time not specified.");
    ...
    */
    function claim(uint256 _txNo) external nonReentrant {
        ...
        require(
            ...
            "HStake: Rewards already claimed."
        );
        require(
            block.timestamp > stakes.stakingPerTx[_txNo].lockedUntil,
            "HStake: Stake period is not over."
        );

        uint256 _reward = rewards(_txNo);
        ...
        stakes.stakingPerTx[_txNo].stakingOver = true;
        habilidash.changeStakeStatus(msg.sender, IHabilidash.StakeStatus.STAKER);

        emit RewardWithdraw(_txNo, amount, _reward);
    }
    ...
    */
    function rewards(uint256 _txNo) public view returns (uint256) {
        ...
        rewardBalance = (stakes.stakingPerTx[_txNo].percent * 10 ** 18);
    ...
    */
    function _addStake(uint256 _time, uint256 _amount) internal {
        ...
        stakes.stakingPerTx[stakes.txNo].lockedUntil =
            block.timestamp +
            _time;
        stakes.stakingPerTx[stakes.txNo].percent = reward[_time];
        if(reward[_time] == 200){
            habilidash.changeStakeStatus(msg.sender, IHabilidash.StakeStatus.ONE_STAKING);
        }
        if(reward[_time] == 500){
            habilidash.changeStakeStatus(msg.sender, IHabilidash.StakeStatus.THREE_STAKING);
        }
        if(reward[_time] == 1200){
            habilidash.changeStakeStatus(msg.sender, IHabilidash.StakeStatus.SIX_STAKING);
        }
